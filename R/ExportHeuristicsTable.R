...
MakeResListDF <- function(res_list1=NULL, out1=NULL, name="res_list") {
  candidates1 <- sapply(res_list1, function(x) {x[["cand_id"]]!=""}); table(candidates1)
  good_rows1 <- sapply(res_list1[candidates1], function(x) {y <- x[["row"]]; names(y) <- x[["cand_id"]]; return(y)})
  raw_all <- sum(out1$P<=p_thr, na.rm=T)
  raw_cnd <- sum(rownames(out1)[which(out1$P<=p_thr)] %in% good_rows1)
  raw_rej <- sum(rownames(out1)[which(out1$P<=p_thr)] %in% sapply(res_list1[!candidates1], function(x) {x[["row"]]}))
  tmp <- data.frame("x"=c(paste0(nrow(out1)," (",raw_all,")"),
...
if (!is.null(res_list1)) {
  ...
  total1 <- length(res_list1)-sum(sapply(res_list1, function(x) { is.null(x[["err_msg"]]) }))
  tmp1 <- plyr::ldply(sort(table(unlist(sapply(res_list1, function(x) { x[["err_msg"]] })))), function(x) {data.frame("abs"=x, "rel"=round(100*x/total1,2))})
} else {
  r1 <- NULL
  tmp1 <- NULL
}
...
if (!is.null(res_list2)) {
  ...
  total2 <- length(res_list2)-sum(sapply(res_list2, function(x) { is.null(x[["err_msg"]]) }))
  tmp2 <- plyr::ldply(sort(table(unlist(sapply(res_list2, function(x) { x[["err_msg"]] })))), function(x) {data.frame("abs"=x, "rel"=round(100*x/total2,2))})
} else {
  r2 <- NULL
  tmp2 <- NULL
}
...
descr <- data.frame("Description"=c("number of mass pairs detected in preCL (number of significant results at p>0.05 without raw data evaluation)",
