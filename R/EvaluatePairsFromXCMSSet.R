  ...
  out <- plyr::ldply(specific_row, function(i) {
    ...
    plyr::ldply(mz_cand, function(x) {
      ...
      if (length(mzok)>=1) {
        ...
        int_mz1 <- data.frame(lapply(split(pks[1,],inter), function(x) { ifelse(any(is.finite(x)), mean(x, na.rm=T), NA) }), check.names=FALSE); colnames(int_mz1) <- paste0("I1_", colnames(int_mz1))
        int_mz2 <- data.frame(lapply(split(pks[2,],inter), function(x) { ifelse(any(is.finite(x)), mean(x, na.rm=T), NA) }), check.names=FALSE); colnames(int_mz2) <- paste0("I2_", colnames(int_mz2))
        ...
        if (test_int_iso & sum(sapply(split(ratios, inter), function(y) { all(is.finite(y)) }))>=floor(0.5*length(levels(inter)))) {
        ...
        dummy <- data.frame(as.list(rep(NA, 2*length(levels(inter))))); colnames(dummy) <- paste0(rep(c("I1_","I2_"), each=length(levels(inter))), levels(inter))
