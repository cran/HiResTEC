...
GenerateCandXLSX <- function(res_list=NULL, xlsx_file=NULL, rejected=FALSE) {
  ...
  cand <- plyr::ldply(res_list, function(x) {
    ...
    if (test) {
      data.frame("ID"=x[["cand_id"]],
                 ...
                 "Spectrum"=paste(apply(x[["s"]],1,function(y){paste(round(y[1],4),y[2],sep=":")}),collapse=" "),
  ...
  if (!is.null(xlsx_file) && nrow(cand)>=1)  {
    ...
    if(class(tmp)=="try-error") warning("Can't write to Excel-File. Seems to be opened.")
  }
