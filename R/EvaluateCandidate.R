...
EvaluateCandidate <- function(x=NULL, tp=NULL, gr=NULL, dat=NULL, mz_iso=1.003355, dmz=0.005, drt=0.5, dEcut=0, Pcut=0.01, Icut=1000, flux_lib=NULL, flux_lib_masses=NULL, flux_lib_rt_dev=5, method=c("APCI","ESI")[1]) {
  ...
  # evaluate BPCs
  res[["bpc"]] <- lapply(dat, function(x) { getMultipleBPC(x=x, mz_dev = dmz, mz=mz1+(0:ng)*mz_iso, rt=rt, rt_dev = 1+4*drt, smooth = 0) })
  ...
  res[["enr"]] <- t(plyr::ldply(res[["bpc"]], function(x) { x[attr(x,"maxBPC"),,drop=FALSE] }))
  attr(res[["enr"]],"Enrichment") <- sapply(res[["bpc"]], function(x) { CalcEnrichment(unlist(x[attr(x,"maxBPC"),])) })
  ...
  median_mass_shift <- apply(sapply(res[["bpc"]],attr,"mass_defect"),1,function(x){ sapply(split(x,tp),median,na.rm=T) })
  mass_drift <- apply(median_mass_shift, 2, function(y) { y[length(y)]-y[1] })
  ...
  if (method=="ESI") {
    ...
    flt <- apply(apply(res[["enr"]], 2, function(y) { if(all(is.na(y))) return(y) else return(y/max(y,na.rm=T)) }), 1, function(z) { sum(z>0.1,na.rm=T)>=0.5*length(z) })
