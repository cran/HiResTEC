...
#'
#'@description
#'\code{CandidateBoxplot} will plot enrichment of tracer incorporationn candidates.
#'
#'@details
#'Plot will be annotated with mz/rt information plus ANOVA(lm()) P-values.
#'
#'@param res Candidate evaluation result.
#'
#'@return
#'A list with all important information including deconvoluted spectrum and linear model.
#'
...
CandidateBoxplot <- function(res=NULL) {
  ...
  graphics::axis(1, at=1:length(unique(tp[flt])), labels = sort(unique(tp[flt]))); graphics::axis(2); graphics::box()
  #browser()
  ...
  if (all(res[["enr"]][1,flt]>0, na.rm=T)) {
    plot(enrg[flt] ~ res[["inter"]][flt], col=sapply(split(sam[flt,"cols"],res[["inter"]][flt]),function(x){ifelse(length(x)>=1,unique(x),0)}), ylab="", main=res[["FluxLib"]], xlab="Interaction Group*Time")
    if (class(res[["lm"]])=="lm") {
